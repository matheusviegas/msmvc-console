...
class FileManipulate{
	...
	private $fileContent;
	private $fileReplace;
	private $fileReplaceLower;
	private $fileArrayReplace;
	private $prefixPath;

	const REPLACE_DEFAULT = "REPLACE_NAME_HERE";
	const REPLACE_DEFAULT_LOWER = "REPLACE_NAME_LOWER_HERE";
	...
	public function createFile($fileName, $fileContent, $prefixPath, $replace = true, $arrayReplace = false){
		...
		if($replace === true){
			...
			$this->fileReplaceLower = self::REPLACE_DEFAULT_LOWER;
	...
	private function renderFile(){
		...
		if(!is_null($this->fileReplace) && !is_null($this->fileReplaceLower)){
			$fileContent = str_replace($this->fileReplace, $this->fileName, $fileContent);
			$fileContent = str_replace($this->fileReplaceLower, strtolower($this->fileName), $fileContent);
		}
